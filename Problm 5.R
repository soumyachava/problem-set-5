...
#   Top contributors to PC1:        PC1          PC2
...
#  Top contributors to PC2:         PC1          PC2
       #fractal_dimension_mean  -0.06436335 -0.366575471
       #fractal_dimension_se    -0.10256832 -0.280092027
       #fractal_dimension_worst -0.13178394 -0.275339469
       #radius_mean             -0.21890244  0.233857132
...
# Silhouette Score
# Perform K-means clustering with 2 clusters (from Elbow Method suggestion)
...
tsne_data_combined <- data.frame(
  ...
  Diagnosis = factor(data$diagnosis)  # True labels
)
set.seed(123)
...
# does Combinig Method work better?
  # The individual methods (PCA, t-SNE, and k-means) show very similar performance with an accuracy of around 91% and a Kappa value of 0.804, indicating a good level of agreement with the true labels.
  # The sensitivity and specificity are also high for all individual methods, with sensitivity at 96% and specificity around 82.5% for each.
  # However, combining methods gives slightly lower accuracy (90.33%) compared to individual methods, but it still maintains a very high performance.
  # The combined method has slightly reduced sensitivity (90.2%) and specificity (90.57%) compared to individual methods, though the improvement in balanced accuracy (0.9038) suggests that combining methods helps balance the detection of both classes more effectively.
  # Combining methods does not significantly improve accuracy in this case, but it may offer better balance in detecting both classes, especially in applications where balanced accuracy is more important.
